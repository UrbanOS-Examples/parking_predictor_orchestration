...
node ('infrastructure') {
    ansiColor('xterm') {
        ...
        doStageUnlessRelease('Build') {
            image = docker.build("scos/predictive-parking-etl:${env.GIT_COMMIT_HASH}")
        }
        ...
        doStageIfRelease('Deploy to Production') {
            ...
            scos.withDockerRegistry {
                image = scos.pullImageFromDockerRegistry("scos/predictive-parking-etl", env.GIT_COMMIT_HASH)
